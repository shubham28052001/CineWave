...
import { fetchTvShows, fetchTopRatedTvShows, fetchAiringToday, fetchOnTheAir } from "../Redux/MovieSlice"
...
export default function TrendingTvShows() {
    ...
    const { Popular, TopRatedTv, AiringToday, OnTheAir, isLoading,error } = useSelector((state) => state.movies)

    const moviesToShow = useMemo(() => {
        switch (active) {
            case 'Popular': return Popular;
            case 'Top Rated': return TopRatedTv;
            case 'Airing Today': return AiringToday;
            case 'On The Air': return OnTheAir;
            default: return Popular;
        }
    }, [active, Popular, TopRatedTv, AiringToday, OnTheAir]);
    ...
    useEffect(() => {
        const fetchActions = {
            'Popular': fetchTvShows,
            'Top Rated': fetchTopRatedTvShows,
            'Airing Today': fetchAiringToday,
            'On The Air': fetchOnTheAir
        };
    ...
    return (
        <div className="min-h-screen bg-black text-white">
            ...
            <div className="pt-20 max-w-7xl mx-auto px-6">
                <h1 className="text-3xl font-black mb-6">Tv Shows</h1>
                ...
                <div className='flex gap-4 flex-wrap pb-2 scrollbar-hide no-scrollbar'>
                    ...
                    <FilterButton
                        label="Airing Today"
                        isActive={active === 'Airing Today'}
                        onClick={() => setActive('Airing Today')}
                    />
                    <FilterButton
                        label="On The Air"
                        isActive={active === 'On The Air'}
                        onClick={() => setActive('On The Air')}
                    />
